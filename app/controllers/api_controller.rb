...
class ApiController < ActionController::Base
    ...
    def getdeck
        if params[:deck_action] == "own"
            ...
            @deckName = ShareDeck.joins("LEFT JOIN deck_names ON deck_names.id = share_decks.deck_name_id").select("share_decks.user_id", "share_decks.share_user_id", "deck_names.deck_name").where(share_user_id: params[:user_id])
