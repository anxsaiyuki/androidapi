...
class ApiController < ActionController::Base
    ...
    def editdeck
        cardtotal = Decklists.find_by_user_id_and_card_id_and_deck_name(params[:user_id], params[:card_id], params[:deck_name])
        if cardtotal.nil?
            deck = DeckLists.create(user_id: params[:user_id], card_id: params[:card_id], deck_name: params[:deck_name], card_quantity: params[:card_quantity])
            ...
            if cardtotal.card_type != "GENERATION" && cardtotal.card_quantity == 3
                ...
                Decklists.find(cardtotal.id).update_attributes(card_quantity: newCardQuantity)
                render json: {message: 'update'}, status: 200
            end
        end
